...
extern struct ModbusRegister DataBuffer[];
...
//    DataLength++;
	...
	TempData = TempForTxBuffer.Word - RTU_REG_BASE; 							//
    count = ModbusRxData[RTU_PAR_NUM_LO]<<1;
    ...
    CRCData=CRC16Bit(&ModbusTxData[0],count+3);
	TempForTxBuffer.Word = CRCData;
    ModbusTxData[count+3] = TempForTxBuffer.Bytes.MSB;  
    DataLength++;
    ModbusTxData[count+4] = TempForTxBuffer.Bytes.LSB; 
    DataLength++;
/*通过串口发送数据*/
	HAL_UART_Transmit(&huart2,ModbusTxData,count+5,1000);	
}
...
//-------------------------------------------
    ...
    ModbusTxData[DataLength]   =  CRCData.Bytes.MSB; 
    DataLength++;
	ModbusRxData[DataLength]   =  RTU_END_BYTE_HI;
	DataLength++;
	ModbusRxData[DataLength]   =  RTU_END_BYTE_LO;
//------------------------------------------------
	HAL_UART_Transmit_DMA(&huart2,ModbusTxData,DataLength); 
}
//===============================================================
